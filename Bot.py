...
def energy_pick(call):
    ...
    bot.send_message(
        chat_id,
        "Напиши 3–7 действий, каждое с новой строки.\n"
        "Пример:\n"
        "1) написать 1 абзац отчёта\n"
        "2) позвонить клиенту\n"
        "3) разобрать почту 10 минут"
    )
...
def get_actions(message):
    ...
    if not 3 <= len(lines) <= 7:
        bot.send_message(
            chat_id,
            "Нужно 3–7 действий. Каждое с новой строки.\n"
            "Пример:\n"
            "• написать 1 абзац отчёта\n"
            "• позвонить клиенту\n"
            "• разобрать почту 10 минут"
        )
        return
